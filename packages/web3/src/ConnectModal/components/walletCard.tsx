...
const CardItem: React.FC<{ icon: ReactNode, title: string, desc: string, link?: string, onClick?: () => void }> = ({
    icon,
    title,
    desc,
    link,
...
const WalletCard: React.FC<WalletCardProps> = (props) => {
    ...
    const selectedExtension = selectedWallet?.extensions ? selectedWallet?.extensions?.[0] : undefined;
    return (
        <>
