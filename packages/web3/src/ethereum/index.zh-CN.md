...
可向用户钱包添加自定义网络（EIP-3085 `wallet_addEthereumChain`），或在切换链时若钱包中不存在该网络则先添加再切换。使用 `@ant-design/web3-wagmi` 的 `useAddNetwork` 与 `useSwitchNetwork`，二者均使用当前已连接钱包的 provider，因此支持任意已连接钱包（MetaMask、WalletConnect、OKX 等）。文案使用 `@ant-design/web3-common` 默认 locale 下的 `AddOrSwitchNetwork`。

<code src="./demos/add-or-switch-network.tsx"></code>

不依赖 `WagmiWeb3ConfigProvider` 的添加网络：使用 `@ant-design/web3-wagmi` 的 `addNetworkToWallet(params, options?)`，仅基于 EIP-6963，可在未包在 Provider 的页面（如独立落地页）中直接使用。
